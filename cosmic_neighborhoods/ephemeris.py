...
def sun_ra_deg(date_gregorian: str) -> float:
    ...
    This calculates the Sun's right ascension at 12:00 UTC (noon) on the given date.
    For cosmic neighborhood assignment, we'll add 180° to this value to get the
    RA that culminates at local midnight.

    The noon time is chosen because:
    1. It's a standard reference time for astronomical calculations
    2. It minimizes the effect of the Sun's daily motion (~1°/day)
    3. It matches JPL Horizons and other ephemeris systems
    ...
    try:
        # Parse date and create noon UTC timestamp
        date = datetime.strptime(date_gregorian + " 12:00:00", "%Y-%m-%d %H:%M:%S")
        time = Time(date, scale="ut1")
